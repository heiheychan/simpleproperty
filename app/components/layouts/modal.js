...
export default function Modal({ children, open, setOpen, formName }) {
  ...
  return (
    <div className={`fixed top-0 left-0 w-screen h-screen ${open ? "flex": "hidden"} justify-center items-center`}>
      <div className="w-full h-full absolute bg-gray-500 opacity-50 z-50" onClick={() => {setOpen(false)}}></div>
      <div className="absolute z-50 bg-black rounded-lg border-gray-800 border min-w-[390px]">
